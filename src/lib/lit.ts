...
export const initLitClient = async () => {
  try {
    ...
    const errorMessage =
      err instanceof Error ? err.message : "Failed to initialize Lit client";
...
export const encryptContent = async (
  ...
  try {
    ...
    const errorMessage =
      err instanceof Error ? err.message : "Unknown error during encryption";
    throw new Error(errorMessage);
  }
};

export const deletageUse = async (
...
export const generateSessionSignature = async (
  ...
  const sessionSigs = await litNodeClient.getSessionSigs({
    chain: LIT_CONFIG.NETWORK,
    expiration: new Date(Date.now() + 1000 * 60).toISOString(), // 1 minute
