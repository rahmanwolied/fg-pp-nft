...
import { NFT_CONFIG } from '@/lib/env'
...
export function NFTMinterForm() {
  ...
  const { encryptContent, createNftOwnershipAccessControlConditions, isEncrypting } = useLitProtocol()
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      // Step 1: Create access control conditions for NFT ownership
      // This example uses a simple condition that checks if the user owns any NFT from the contract
      // See: https://developer.litprotocol.com/concepts/access-control-concept
      const accessControlConditions = createNftOwnershipAccessControlConditions(
        NFT_CONFIG.CONTRACT_ADDRESS,
        parseInt(NFT_CONFIG.CHAIN_ID)
      )

      // Step 2: Encrypt the content using Lit Protocol
      ...
      const { url } = await uploadToIPFS(JSON.stringify(metadata), `${values.name.replace(/\s+/g, '-')}.json`)

      // Step 5: Add to local state (in a real app, you would mint the NFT here)
      const newNFT = {
        id: `nft-${Date.now()}`,
        name: values.name,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Enter text content to mint as an NFT. Your content will be encrypted using Lit Protocol
          and stored on IPFS.
        </CardDescription>
      </CardHeader>
      <CardContent>
